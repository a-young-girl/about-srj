...
/*//只有一面
	var fileList1=tools.$('.file-item');
	for (var i = 0; i < fileList1.length; i++) {
		tools.addEvent(fileList1[i],'click',function(ev){
		var target=ev.target;//事件源
		if (tools.parents(target,'.item')) {
			target=tools.parents(target,'.item');
			//找到文件id
	...
	mouseover2();//鼠标放文件区域背景高亮显示小框框
	//mouseover
	function mouseover2(){
		var item=tools.$('.item');
		var checked=tools.$('.checkbox');
		for (var i = 0; i < item.length; i++) {
			item[i].index=i;
			//alert(checked.length);//5
			tools.addEvent(item[i],'mouseover',function(ev){
				var target=ev.target;
				checked[this.index].style.display='block';
				item[this.index].style.background='#e7f2fe';
				return target;
			});
			tools.addEvent(item[i],'mouseout',function(ev){
				var target=ev.target;
				checked[this.index].style.display='none';
				item[this.index].style.background='transparent';
				return target;
			});
			...
			//alert(checked.length);//5
			checked[i].index=i;
			tools.addEvent(checked[i],'click',function(ev){
				ev.stopPropagation();
				var target=ev.target;
				checked[this.index].style.backgroundPosition='0px -421px';//background-position: 10px -310px;
				item[this.index].style.background='#e7f2fe';
				return target;
			});
		}
